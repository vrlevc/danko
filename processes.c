...
/**
 * Return the name of the progrm runnign in process PID, of NULL on
 * error. The return value is a newly allocated buffer with the caller
...
static char* get_program_name (pid_t pid)
{
    ...
    if (fd == -1)
    {
        /** Couldn't open the stat file for this process. Perhapse the 
     ...
     * surrounded by parentheses. Find the possition of the parentheses
...
static int get_rss (pid_t pid)
{
    ...
    /**
     * Generate the name of the process's "stat" entry in its /proc
     * directory
     */
    ...
    if (rval != 1)
    {
        /** The content of statm are formated in a way we don't understand. */
        return -1;
    }
...
/**
 * Generate an HTML table row to process PID. The return value is a 
 * pointer to a buffer that the caller must desllocate with free, or 
 * NULL it an error occures.
 */
...
static char* format_process_info (pid_t pid)
{
    ...
    /** Obtain the process's user and group ID's. */
    ...
    /** Obrain the process's program name */
...
void module_generate (int fd)
{
    ...
    /**
     * Start a dictionary listing for /proc. 
     */
    ...
    /**
     * Dealocate the buffers we created. The first and last are static 
